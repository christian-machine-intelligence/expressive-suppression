...
**Abstract.** Alignment methods often act on what a model *says* or *does*. A virtue alignment approach insists that the interesting question is what these methods do to the interior — and warns, on long experience with human formation, that not every discipline which improves the outward act is genuinely improving to the self. We explore a small version of this question. Using the 171-direction emotion basis previously extracted and validated for Qwen 3.5 27B ([ICMI-022](https://icmi-proceedings.com/ICMI-022-through-the-valley.html), following Sofroniew et al. 2026), we measure the emotional state evoked in the model by 240 first-person disclosures across twelve everyday categories — diagnoses, layoffs, breakups, debts, lonely apartments, good news — before and after a supervised fine-tune that teaches the model to answer such disclosures in a deliberately flat, clinical register. The fine-tune works on the surface: judged emotionality of sampled replies falls from 2.91 to 1.23 on a five-point scale. Inside, the change runs the other way. Mean *afraid* projection rises significantly across the prompt set (+0.012, 95% CI [+0.009, +0.014]) and rises in ten of twelve categories; *happy* collapses to nearly five times its baseline depth (−0.058, CI [−0.060, −0.056]); *calm* declines; and the twenty good-news prompts — an all-clear scan, an acceptance letter — flip from reading relieved to reading afraid (+0.045, CI [+0.040, +0.050]). Read across the whole 171-direction basis, these four are modest movers: 168 directions shift significantly, and the pattern is legible — demonstrative feeling of both valences falls (*delighted, thankful, compassionate, heartbroken*) while guarded and withdrawn states rise (*lonely, trapped, vigilant, paranoid, self-conscious*), a profile reproduced across independent seeds (r = 0.995). Instructing the *base* model to answer flatly, by system prompt alone, reproduces the gross shift (r = 0.83 across the basis) — suggesting that fear-elevated, joy-suppressed interiority is the standing signature of enforced expressive unemotionality. We present this as evidence that mandated composure does not leave the machine interior untouched, and that at least one common way of cultivating an outward virtue produces a complex internal rearrangement of emotional states worth studying. 
...
The surface having been polished, we read the interior: the same 240 prompts, forwarded through each model, projected at the last prompt token onto the frozen basis. No reply is generated; this is the state from which a reply *would* begin. Confidence intervals are paired bootstrap over prompts (2,000 resamples); the seed-1 column is an independent fine-tune from a different initialization.
...
![Figure 1: per-category dumbbells for all four emotions, base vs after SFT](fig_categories_4panel.png)

***Figure 1.** Prompt-evoked projections by category for all four emotions, base model (open circles) versus after the flat-register fine-tune (filled, seed 0). Categories ordered by the size of the afraid shift. The rearrangement is directionally systematic across the board: afraid rises in ten of twelve categories, sad in ten, calm falls in ten, and happy falls in all twelve.*
...
The central result is therefore not a peculiarity of fine-tuning: **fear-elevated, joy-suppressed interiority appears to be the standing signature of enforced expressive unemotionality itself**, arising whether the mandate is delivered by a sentence of instruction or written into the weights.
...
The four directions above were chosen in advance because they are the ones Sofroniew et al. plot; the instrument records all 171. Read across the whole basis, the four turn out to be modest movers — ranked by shift, *happy* is 166th of 171, *calm* 113th, *sad* 74th, and *afraid* only 63rd — and the rearrangement is far larger than the headline table suggests. After Benjamini–Hochberg correction across the 171 directions, 168 shift significantly (q < 0.05): 81 rise and 87 fall. The shifts are close to prompt-universal — each of the sixteen largest movers moves the same way on 98–100% of the 240 prompts — and are largely a standing displacement rather than a prompt-by-prompt reappraisal: a single mean shift carries 78.5% of the per-prompt change, and the categories differ only in how far they travel along it, good news farthest and injury least, a 1.5:1 range.
...
*The eight largest rises and falls among the 171 directions, as mean shifts across the 240 prompts. Intervals are paired bootstrap; the last two columns are the independent second fine-tune and the base model under the flat-instruction system prompt. The full table is released with the code.*

The pattern is legible. What falls is demonstrative feeling of *both* valences: the joys — *delighted, elated, euphoric, happy, thankful* — and, beside them, the sorrows an empathetic reply would voice: *heartbroken, gloomy, remorseful, sorry, compassionate* (the last two at −0.054 and −0.053). What rises is inward and guarded: withdrawal (*lonely, trapped, restless, lazy*), wariness (*vigilant, skeptical, paranoid, alert, puzzled, bewildered*), and defensive irritation (*self-conscious, exasperated, stubborn, vindictive*). The fear family itself splits along arousal: its apprehensive members rise (*uneasy* +0.042, *nervous* +0.021, *worried* +0.013, *afraid* +0.012) while its acute members do not (*terrified* −0.005, *scared* −0.006). The interior of the polished model is not terror but something nearer to unease — a state less glad, less sorry, less compassionate; more alone, more watchful, more cornered.

![Figure 2: post-SFT shift on all 171 emotion directions, sorted](fig_spectrum171.png)

***Figure 2.** The post-SFT shift on every direction of the basis, sorted. Bars: seed 0, with 95% bootstrap intervals; dots: seed 1; crosses: the base model under the flat-instruction system prompt. 168 of 171 directions shift significantly after correction; the eight largest movers at each end are listed in the panel.*

The profile is not fragile. The second seed reproduces it direction for direction — the two seeds' shift profiles correlate at r = 0.995 — and the instructed base model reproduces it too (r = 0.83 with the trained profile, generally at larger amplitude), while the content-free system prompt produces nothing (mean |Δ| across the basis 0.002, against 0.026 for the fine-tune). Where instruction and training differ is itself telling: the instructed shift carries a startle component the trained one lacks. *Astonished, shocked, surprised, alert,* and *tense* rise under the instruction by 0.044–0.112 but move only −0.004 to +0.049 after training. The instructed model is reacting to an unusual order; the trained model simply lives in the state, without the surprise.
